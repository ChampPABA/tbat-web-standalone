...
import { handleError, safeDatabaseOperation } from "./error-handler";
...
const CODE_LENGTH = 4; // Updated to 4 characters as per Story 3.1 requirements
...
export async function generateExamCode(
  ...
  const { packageType, subject, userId, sessionCapacityId, sessionTime } = options;
  ...
  try {
    ...
    // Convert sessionTime to enum value
    const sessionTimeEnum = sessionTime === "13:00-16:00" ? "AFTERNOON" : "MORNING";
    ...
    await handleError(error, {
      operation: 'generateExamCode',
      component: 'exam-code',
      userId,
      metadata: { packageType, subject, attempts }
    });
...
export async function validateExamCode(code: string): Promise<boolean> {
  const freePattern = /^FREE-[A-Z0-9]{4}-(BIOLOGY|CHEMISTRY|PHYSICS)$/;
  const advancedPattern = /^ADV-[A-Z0-9]{4}$/;
...
export async function getExamCodeDetails(code: string) {
  try {
    ...
    // Check if exam code has expired (assuming 24 hours validity from creation)
    const expiryTime = new Date(examCode.createdAt);
